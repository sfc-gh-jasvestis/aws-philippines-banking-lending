-- ============================================================================
-- 09_AWS_INTEGRATION.SQL — AWS services for Digital Lending & Credit Scoring Platform
...
CREATE OR REPLACE EXTERNAL ACCESS INTEGRATION aws_philippines_banking_lending_BEDROCK_EAI
  ALLOWED_NETWORK_RULES = (DIGITAL_LENDING.APP.BEDROCK_NETWORK_RULE)
  ALLOWED_AUTHENTICATION_SECRETS = (DIGITAL_LENDING.APP.AWS_BEDROCK_SECRET)
  ENABLED = TRUE
  COMMENT = 'Bedrock access for Digital Lending & Credit Scoring Platform';
...
CREATE OR REPLACE FUNCTION APP.BEDROCK_GENERATE(prompt VARCHAR)
  ...
  EXTERNAL_ACCESS_INTEGRATIONS = (aws_philippines_banking_lending_BEDROCK_EAI)
  SECRETS = ('aws_creds' = DIGITAL_LENDING.APP.AWS_BEDROCK_SECRET)
AS $$
...
-- Stream ARN: arn:aws:kinesis:ap-southeast-1:<YOUR_AWS_ACCOUNT_ID>:stream/aws-philippines-banking-lending-stream

CREATE OR REPLACE PIPE RAW.REALTIME_PIPE
  AUTO_INGEST = TRUE
  INTEGRATION = 'aws_philippines_banking_lending_S3_INT'
